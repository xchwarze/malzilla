...
uses
  Windows, Messages, SysUtils, {$IFDEF D6OR7}Variants, {$ENDIF}Classes,
  Graphics, Controls, Forms,
  Dialogs, StdCtrls, ComCtrls, js15decl, ExtCtrls, Menus, Clipbrd;

type
  TDebugType = (dtError, dtBreakpoint, dtTrace);
  TDebugResult = (dtContinue, dtStepInto, dtStepOver, dtAbort);
  TDebugExecuteProc = function(const Code: string; Frame: PJSStackFrame): string
    of object;
  ...
  TDebugMain = class(TForm)
    ...
    procedure lvCallstackSelectItem(Sender: TObject; Item: TListItem; Selected:
      Boolean);
  ...
  public
    ...
    procedure AddCallStack(const Fun, Script: string; Frame: PJSStackFrame;
      Line: Integer);
...
procedure TDebugMain.AddCallStack(const Fun, Script: string; Frame:
  PJSStackFrame; Line: Integer);
var
  p: PStackItem;
  Item: TListItem;
begin
...
function TDebugMain.AddSubProperty(Parent: TTreeNode; const Value: string):
  TTreeNode;
begin
  Result := tvVars.Items.AddChild(Parent, Value);
end;

procedure TDebugMain.lvCallstackSelectItem(Sender: TObject; Item: TListItem;
  Selected: Boolean);
var
  Data: PStackItem;
begin
...
procedure TDebugMain.mnuCopyClick(Sender: TObject);
var
  s: string;
begin
  s := tvVars.Selected.Text;
  Clipboard.AsText := s;
end;
